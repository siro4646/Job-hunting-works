...
namespace ym
{
	...
	// 初期化する
	//---------------------------------------
	bool MeshShapeInstance::Initialize(ym::Device *pDev, ym::CommandList *pCmdList, const MeshShape *shape, const void *p_vertex_head)
	{
		...
		auto vbInitFunc = [&](VertexBuffer &vb, size_t stride, size_t offset)
			{
				if (!vb.buffer_.Initialize(pDev, stride * shape->numVertices, stride, ym::BufferUsage::VertexBuffer, false, false))
				{
					return false;
				}
	...
	// 初期化する
	//---------------------------------------
	bool MeshSubmeshInstance::Initialize(ym::Device *pDev, ym::CommandList *pCmdList, const MeshSubmesh *submesh, const void *p_vertex_head)
	{
	...
	// 初期化する
	//---------------------------------------
	bool MeshInstance::Initialize(ym::Device *pDev, ym::CommandList *pCmdList, const void *pBin)
	{
		assert(pDev != nullptr);
	...
	{
		ym::SafeDeleteArray(pShapes_);
		ym::SafeDeleteArray(pSubmeshes_);
		pHead_ = nullptr;
		pMaterials_ = nullptr;
	}
