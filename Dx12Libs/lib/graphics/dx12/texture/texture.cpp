...
namespace ym
{
	...
	//----
	bool Texture::InitializeFromDDS(Device *pDev, CommandList *pCmdList, const void *pDdsBin, size_t size, ym::u32 mipLevels, bool isForceSRGB, bool forceSysRam)
	{
	...
	//----
	bool Texture::InitializeFromTGA(Device *pDev, CommandList *pCmdList, const void *pTgaBin, size_t size, ym::u32 mipLevels, bool isForceSRGB, bool forceSysRam)
	{
	...
	//----
	bool Texture::InitializeFromPNG(Device *pDev, CommandList *pCmdList, const void *pPngBin, size_t size, ym::u32 mipLevels, bool isForceSRGB, bool forceSysRam)
	{
	...
	//----
	bool Texture::InitializeFromEXR(Device *pDev, CommandList *pCmdList, const void *pPngBin, size_t size, ym::u32 mipLevels, bool forceSysRam)
	{
	...
	//----
	std::unique_ptr<DirectX::ScratchImage> Texture::InitializeFromEXRwoLoad(Device *pDev, const void *pExrBin, size_t size, ym::u32 mipLevels)
	{
		if (!pDev)
		{
			return std::unique_ptr<DirectX::ScratchImage>();
	...
	//----
	bool Texture::InitializeFromHDR(Device *pDev, CommandList *pCmdList, const void *pPngBin, size_t size, ym::u32 mipLevels, bool forceSysRam)
	{
	...
	//----
	std::unique_ptr<DirectX::ScratchImage> Texture::InitializeFromHDRwoLoad(Device *pDev, const void *pTgaBin, size_t size, ym::u32 mipLevels)
	{
		if (!pDev)
		{
			return std::unique_ptr<DirectX::ScratchImage>();
		...
		// TGAファイルフォーマットからイメージリソースを作成
		std::unique_ptr<DirectX::ScratchImage> image(new DirectX::ScratchImage());
		auto hr = DirectX::LoadFromHDRMemory((const u8 *)pTgaBin, size, nullptr, *image);
		if (FAILED(hr))
		{
			return nullptr;
		}
